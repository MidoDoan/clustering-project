...
private:
  ...
  double CalculateWaitingTime (uint64_t neighbor_num, uint64_t node_num, double distance);
  ...
  void SetTwaitArr ();
  void SendFormation ();
  void ReBroadcastDataPacket (uint64_t channel, uint64_t seq, uint64_t sender_id, Ptr<Packet> pkt);
...
WaveNetDeviceExample::CreateWaveNodeCustomMobility (std::string file_path)
{
  ...
  nodes = posInfo.GetContainer (1, 0, posInfo.GetNodeNum ());
  ...
  delay_mat.InitiateMatrix (devices.GetN (), devices.GetN ());
          ...
          if (nodeInfoList[sender_id].degree != ClusterSupport::CM)
            {
              ...
              // Vector pos = nodes.Get (sender_id)
              //                  ->GetObject<ConstantVelocityMobilityModel> ()
              //                  ->GetPosition ();
              // Vector vel = nodes.Get (sender_id)
              //                  ->GetObject<ConstantVelocityMobilityModel> ()
              //                  ->GetVelocity ();
              // std::cout << pos.x << std::endl;
              // ClusterSupport::NodeInfo clusterInfo;
              // clusterInfo.nodeId = sender_id;
              // clusterInfo.degree = nodeInfoList[sender_id].degree;
              // clusterInfo.clusterId = sender_id;
              // clusterInfo.posX = pos.x;
              // clusterInfo.posY = pos.y;
              // clusterInfo.posZ = pos.z;
              // clusterInfo.velX = vel.x;
              // clusterInfo.velY = vel.y;
              // clusterInfo.velZ = vel.z;
              Mac48Address dest = Mac48Address::GetBroadcast ();
              Ptr<Packet> p = Create<Packet> (100);
      ...
      if (Now ().GetSeconds () < ClusterFormationTime + BeaconExTime + 0.1 + DataExchangeTime)
        {
          ...
          Ptr<WaveNetDevice> clusterHead = DynamicCast<WaveNetDevice> (devices.Get (nodeInfoList[sender_id].clusterId));
          ...
          sender->Send (p, clusterHead->GetAddress (), 0);
        }
      else
...
{
  Simulator::Schedule (time, &WaveNetDeviceExample::SendPacket, this, channel, sender_id, 1, 0);
}
void
WaveNetDeviceExample::ScheduleSendPacket1 (Time time, uint64_t channel, uint64_t sender_id)
{
  Simulator::Schedule (time, &WaveNetDeviceExample::SendPacket, this, channel, sender_id, 2, 0);
}
  ...
  if (item.tid.GetName () == "ns3::BeaconExHeader")
  ...
  else 
    {
      ...
      std::cout << "Receive data packet: " << std::endl
                << " Sender: " << dataHeader.GetSeq () << std::endl
      ...
      //     Simulator::Schedule (Seconds(Now ().GetSeconds ()), &WaveNetDeviceExample::ReBroadcastDataPacket,
      //                          this, dataHeader.GetSeq (), node_pos, p);
      //   }
    }
  // else
...
WaveNetDeviceExample::SendPacketExample ()
{
  ...
  CreateWaveNodeCustomMobility ("position_3_1_10_600_50_new.txt");
  ...
  for (uint64_t i = 0; i != devices.GetN (); ++i)
    {
      ...
      Simulator::Schedule (Seconds (BeaconExTime + ClusterFormationTime + 0.1 + DataExchangeTime),
                           ...
                           sender, SCH1);
      Simulator::Schedule (Seconds (BeaconExTime + ClusterFormationTime + 0.1 + DataExchangeTime),
                           &WaveNetDevice::StopSch, sender,
                           SCH1);
      Simulator::Schedule (Seconds (BeaconExTime + ClusterFormationTime + 0.1 + DataExchangeTime),
                           &WaveNetDevice::StopSch, receiver,
                           SCH1);
    }
  ...
  //   }
  // SendFormation ();
  // Simulator::Schedule (Seconds (BeaconExTime + ClusterFormationTime + 0.1 + 0.1),&WaveNetDeviceExample::SendPacket,this, SCH1, 1, 3, 2);
  // for (uint64_t i = 0; i != devices.GetN (); ++i)
  //   {
  //     Simulator::Schedule (Seconds (BeaconExTime + ClusterFormationTime + 0.1 + DataExchangeTime),
  //                          &WaveNetDevice::DeleteTxProfile,
...
double
WaveNetDeviceExample::CalculateWaitingTime (uint64_t node_id, uint64_t node_num, double distance)
{
