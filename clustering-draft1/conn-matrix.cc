...
namespace ns3 {
void
ConnectivityMatrix::InitiateMatrix (uint32_t row_num, uint32_t col_num)
{
...
void
ConnectivityMatrix::GetReceiveNodes (uint32_t sender_id)
{
  uint64_t count = 0;
  for (uint64_t i = 0; i < matrix.size (); i++)
    {
...
uint32_t
ConnectivityMatrix::GetNeighborNum1 (uint32_t node_id)
{
  std::vector<uint32_t> row;
  for (uint32_t i = 0; i < matrix.size (); i++)
    {
...
void
ConnectivityMatrix::GetTotalPDR (std::vector<uint64_t> packet_src, uint32_t src_num)
{
      ...
      if (nodePDR[i].size () != 0)
        {
          avgNodePDR.push_back (std::accumulate (nodePDR[i].begin (), nodePDR[i].end (), 0.0) /
                                nodePDR[i].size ());
          // std::cout << std::accumulate (nodePDR[i].begin (), nodePDR[i].end (), 0.0) /
          //                  nodePDR[i].size ()
          //           << std::endl;
        }
    }
  for (uint32_t i = 0; i < avgNodePDR.size ();i++){
      std::cout << avgNodePDR[i] << " ";
    }
  std::cout << "" << std::endl;
  double totalAVGPDR =
      std::accumulate (avgNodePDR.begin (), avgNodePDR.end (), 0.0) / avgNodePDR.size ();
  std::cout << "The total avg PDR: " << totalAVGPDR << std::endl;
  // for (uint32_t i = 0; i < avgNodePDR.size (); i++)
  //   {
  //     std::cout << "The avg PDR of node " << i << ": " << avgNodePDR[i] << std::endl;
  //   }
}
} // namespace ns3
